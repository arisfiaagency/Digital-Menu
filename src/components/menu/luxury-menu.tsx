...
import { MenuItemDetailModal } from "@/components/menu/menu-app";
...
import { menuThemeCss } from "@/lib/utils/color";
import { useLocale } from "@/hooks/use-locale";
import type { AppData, MenuItem } from "@/types/models";

// Serif display stack for the luxury skin. The cafe's heading font comes first
// (so Arabic/Kurdish scripts stay correct), with a serif fallback for Latin.
const LUX_DISPLAY = "var(--menu-heading-font), Georgia, 'Times New Roman', serif";
...
// The "Luxury" structural skin: an editorial, gallery-style menu — full hero,
// centered letter-spaced index nav, hairline-ruled sections, and refined item
// rows with reveal-on-scroll motion. It shares the cart, item detail modal, and
// data helpers with the default skin but owns its whole layout.
export function LuxuryMenu({
  initialCategorySlug,
  initialData
}: {
  ...
  const { locale, setLocale, dir: textDir } = useLocale(defaultAppData.general.defaultLanguage, { documentDirection: "ltr" });
  ...
  const tagline = localized(general.welcomeTagline, locale);
  ...
  const showCart = showPrices && appearance.showCartButton !== false;
  ...
  // Deep link into a category once on load.
  useEffect(() => {
  ...
  }, [initialCategorySlug, data.categories, sections]);
  ...
  return (
    <main dir="ltr" className="menu-theme-root lux-root no-select relative min-h-screen">
      <style dangerouslySetInnerHTML={{ __html: menuThemeCss(appearance) }} />
      {/* Refined layered background derived from the cafe surface + accent. */}
      ...
      <div className="relative z-20">
        <div dir={textDir} className="mx-auto flex w-full max-w-5xl items-center justify-between px-5 py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {darkModeEnabled ? (
              <ThemeToggle
                presentation={appearance.menuThemeToggleStyle ?? "circle"}
                iconStyle={appearance.menuThemeIconStyle ?? "sunMoon"}
              />
            ) : null}
      ...
      <header className="relative z-10 px-5">
        <div className="mx-auto flex max-w-3xl flex-col items-center gap-5 py-12 text-center sm:py-16">
          ...
          {tagline ? <p dir={textDir} className="max-w-xl text-base italic text-muted-foreground sm:text-lg" style={displayStyle}>{tagline}</p> : null}
          {description ? <p dir={textDir} className="max-w-xl text-sm text-muted-foreground">{description}</p> : null}
          <div className="mt-1 flex flex-wrap items-center justify-center gap-3">
            <OpenStatusBadge locale={locale} textDir={textDir} openHour={general.openHour} closeHour={general.closeHour} style={appearance.openStatusStyle ?? "outline"} />
            {appearance.welcomeShowSocialLinks !== false ? <SocialLinks social={general.socialLinks} style={appearance.socialLinkStyle ?? "outline"} /> : null}
          </div>
          {searchEnabled ? (
      ...
      {showCart && cart.totalQuantity > 0 ? (
        ...
        >
          ...
          <span className="tracking-wide">{formatMoney(cartGrandTotal, cart.currency, locale)}</span>
    ...
    <Reveal delay={Math.min(index, 6) * 60} className="group py-6 first:pt-0">
      <div dir={textDir} className="flex items-start gap-5">
        ...
        <button type="button" onClick={onOpen} className="min-w-0 flex-1 text-start">
          ...
          {item.isSoldOut ? (
            <span className="mt-2 inline-block text-[11px] font-semibold uppercase tracking-widest text-destructive">{translate(locale, "menu.soldOut") || "Sold out"}</span>
          ) : null}
        </button>
