...
function RetroCard({
  ...
  return (
    <li className="flex flex-col overflow-hidden rounded-2xl border-2 border-stone-900/10 bg-white/70 dark:border-white/10 dark:bg-stone-900/60">
      {showImages ? (
        <button
          type="button"
          onClick={onOpen}
          aria-label={name}
          className="group relative aspect-square overflow-hidden bg-muted ring-2 ring-primary/20"
        >
          <FallbackMenuImage src={item.imageUrl} alt={name} />
          {item.isSoldOut ? (
            <span className="absolute inset-0 flex items-center justify-center bg-background/60 text-[10px] font-black uppercase tracking-widest text-destructive">
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-3">
        <button type="button" onClick={onOpen} className="min-w-0 text-start">
          <h3 dir={textDir} className="line-clamp-2 text-base font-extrabold uppercase leading-tight tracking-wide">{name}</h3>
          ...
          {!showImages && item.isSoldOut ? (
            <span className="mt-1 inline-block text-[10px] font-black uppercase tracking-widest text-destructive">{translate(locale, "menu.soldOut")}</span>
          ) : null}
        </button>
        <div className="mt-auto flex items-center justify-between gap-2 pt-1">
          {showPrices ? (
            <span className="rounded-lg bg-primary px-2.5 py-1 text-sm font-black tabular-nums text-primary-foreground shadow-sm">
          ...
          {showCart && !item.isSoldOut ? (
            quantity > 0 ? (
              ...
              <button type="button" aria-label={`${translate(locale, "cart.add")} ${name}`} onClick={onAdd}
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border-2 border-primary text-primary transition-colors hover:bg-primary hover:text-primary-foreground">
