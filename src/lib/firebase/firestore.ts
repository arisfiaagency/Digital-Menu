...
/** Supervisor: load reviews for any cafe by slug (platform staff). */
export async function listClientReviews(slug: string, max: number = 500): Promise<Review[]> {
  ...
  const snap = await getDocs(
    query(collection(db, "clients", normalized, "reviews"), orderBy("createdAt", "desc"), limit(max))
  );
  return snap.docs.map((entry) => ({ id: entry.id, ...(entry.data() as Omit<Review, "id">) }));
}
...
export const recordClientPayment = bindClientSlug(async function recordClientPayment(input: {
  ...
  const planPrice = client.subscription?.price || 0;
  const owed = Math.max(0, (client.billing?.amountOwed || 0) - amount);
  const baseExpiry = client.subscription?.expiresAt || client.trial?.endAt;
  const expiresAtAfter = extendSubscriptionExpiry(baseExpiry, months);
  ...
  const payment: PlatformPayment = {
    ...
    monthsAdded: months,
    expiresAtAfter,
    note: input.note?.trim() || (months === 1 ? "1 month" : `${months} months`),
